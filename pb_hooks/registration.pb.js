routerAdd('POST', '/api/seller-number/registration', (e) => {
  ...
  if (!sellerFirstName || !sellerLastName || !sellerEmail) {
    return e.json(400, { error: 'sellerFirstName, sellerLastName, and sellerEmail are required' })
  }
  ...
  try {
    ...
    if (sellerNumber.get('sellerDetails')) {
      return e.json(409, { error: 'Seller number already has associated seller details' })
    }

    // Get the seller number pool to access the event category
    let sellerNumberPool
    try {
      sellerNumberPool = $app.findRecordById('sellerNumberPools', sellerNumber.get('sellerNumberPool'))
    } catch (error) {
    ...
    try {
      sellerNumberVariation = $app.findRecordById('sellerNumberVariations', sellerNumberPool.get('sellerNumberVariation'))
    } catch (error) {
    ...
    let eventCategory
    try {
      eventCategory = $app.findRecordById('eventCategories', sellerNumberVariation.get('eventCategory'))
    } catch (error) {
    ...
    if (sessionTimeInSec) {
      const timeDiff = (new Date().getTime() - new Date(reservedAt).getTime()) / 1000
    ...
    const sellerDetailsCollection = $app.findCollectionByNameOrId('sellerDetails')
