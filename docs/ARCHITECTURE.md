...
- **Output**: `{ eventId, sellers: [{ number, sellerNumberId, markets, firstMarket, lastMarket,
  trail: [{ market, number, itemsSold, revenueCents }] }] }`
...
the first and last of them, and every one of them (newest first) with the number drawn there
and its figures — the Verkäuferliste shows the last five on hover over "dabei seit" and the
whole trail in the edit row. Zero means "neu dabei" as far as the register's trail reaches
